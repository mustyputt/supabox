...
def MAINSG():
        ...
        main.addDir('Latest Episodes','http://seriesgate.tv/latestepisodes/',602,art+'/latest.png')
        HOMESG()
        main.GA("Plugin","SeriesGate")
        #main.addDir('TV Series','http://watch-freeseries.mu/tvseries',506,art+'/series.png')
        #main.addDir('Year','http://watch-freeseries.mu/',505,art+'/year.png')
        #main.addDir('Genre','http://watch-freeseries.mu/',502,art+'/genre.png')
def HOMESG():
...
def LISTEpiSG(murl):
    ...
    for url,thumb,epiname, showname, seep in match:
        ...
        main.addPlayTE(showname.strip()+' '+seep.strip()+" "+'[COLOR blue]'+epiname+'[/COLOR]',durl,609,thumb,'','','','','')
...
def LISTSeasonSG(mname,murl,thumb):
    link=main.OPENURL(murl)
    match=re.compile('(?i)</span></li><li><a href="([^"]+?)">([^<]+?)</a>').findall(link)
    for url, seaname in match:
        url += 'searchresult/'
        seaname = seaname.replace('&raquo','').strip()
        main.addDir(mname+' '+seaname,url,609,str(thumb))
...
def SEARCHSG(murl):
    ...
    for thumb,url,epiname, name in match:
        ...
        main.addDirTE(name+' [COLOR blue]'+epiname+'[/COLOR]',durl,609,thumb,'','','','','')
        loadedLinks = loadedLinks + 1
        percent = (loadedLinks * 100)/totalLinks
        remaining_display = 'Episodes loaded :: [B]'+str(loadedLinks)+' / '+str(totalLinks)+'[/B].'
        dialogWait.update(percent,'[B]Will load instantly from now on[/B]',remaining_display)
