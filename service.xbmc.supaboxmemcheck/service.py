...
import platform,urllib,urllib2,re,xbmcplugin,xbmcgui,xbmc,shutil,time,extract,datetime,settings, xbmcaddon, os, sys
import xbmc
import plugintools
import datetime
...
def ADDONINSTALLER(url):
    ...
    if pluginpath: xbmc.executebuiltin("RunAddon("+url+")")
        ...
        url=supaboxUrl+'app/'+url+'.zip'; path=xbmc.translatePath(os.path.join('special://home','addons','packages')); lib=os.path.join(path, filename+'.zip'); DownloaderClass(url,lib)
        time.sleep(3)
...
def check_stats():
    ...
    try:
        ...
        shorts=re.compile('addon="(.+?)".+?tatus="(.+?)"').findall(link)
        for addon, status in shorts:
            #dialog = xbmcgui.Dialog()
...
def FINDADDON(url,name):  
    ...
    for file in os.listdir(pluginpath):
        if url in file:
                ...
                try: 
                    return adddir(name,file,9,iconimage,fanart)
                except:
...
def REMOVEADDON(url):   
    ...
    xbmc_cache_path = os.path.join(xbmc.translatePath('special://home'),'addons',url)
    #dialog = xbmcgui.Dialog()
