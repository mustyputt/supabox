...
class oss(LiveTVSource,CustomSettings):
    implements = [LiveTVSource,CustomSettings]
    
    display_name = "OFFSIDE STREAMS"
    
    name = 'OFFSIDE STREAMS'
    ...
    def __init__(self):
        xml = '<settings>\n'
        xml += '<category label="OSS Account">\n'
    ...
    def GetFileHosts(self, id, other_names, region, language, list, lock, message_queue):
            ...
            datapath = xbmc.translatePath('special://home/userdata/addon_data/plugin.video.offside')
                ...
                if clean_id in p.lower().replace(' ',''):
                    ...
                    for name,url in match:
                        if 'HD' in name:
                            ...
                            res='SD'
                        self.AddLiveLink( list, name, url, host='OFFSIDE STREAMS',quality=res)
