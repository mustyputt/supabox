...
class tvrage(TVShowIndexer):
    ...
    def ExtractContentAndAddtoList(self, indexer, section, url, type, list, page='', total_pages='', sort_by='', sort_order=''): 
        ...
        if section == 'latest':
            ...
            if page == '':
                ...
                page = str( int(page) )
                new_url = 'http://www.watchseries.to/latest/'+page
            ...
            match=re.compile('<li><a href="/episode/(.+?)">(.+?) Seas. (.+?) Ep. (.+?) \(.+?\)<span class="epnum">(.+?)</span></a></li>').findall(html)
            match2=re.compile('<li><a href="/episode/(.+?)">(.+?) Seas. (.+?) Ep. (.+?) \(.+?\)<.+?</li>').findall(html)
            for url, name, Sea_num, eps_num, date in match:
                for url, name, Sea_num, eps_num in match2:
                    ...
                    self.AddContent(list, indexer, common.mode_File_Hosts, name +'[COLOR royalblue] ('+sea_eps+')[/COLOR] '+'Date Aired: '+date, item_id, 'tv_episodes', url=url, name=name, year=year, season=Sea_num, episode=eps_num)
            ...
            match=re.compile('<li><a href="/episode/(.+?)">(.+?) Seas. (.+?) Ep. (.+?) \(.+?\)</a></li>').findall(html)
    ...
    def GetSection(self, indexer, section, url, type, list, page='', total_pages='', sort_by='', sort_order=''):
        ...
        if section == 'main':
            self.AddSection(list, indexer,'latest','Date Added','http://www.watchseries.to/latest',indexer)
            self.AddSection(list, indexer,'popular','Popular Episodes Added This Week','http://www.watchseries.to/new',indexer)
