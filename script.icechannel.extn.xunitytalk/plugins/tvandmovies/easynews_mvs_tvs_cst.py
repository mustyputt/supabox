...
class EasyNews(MovieSource, TVShowSource, CustomSettings):
    ...
    def GetFileHostsForContent(self, title, name, year, season, episode, type, list, lock, message_queue):                 
        ...
        for field in data:
            ...
            url='members.easynews.com/dl/'+num+codec+'/'+_name+codec
            name_ = '[COLOR orange][FMT: %s, FSZ: %s][/COLOR] %s' %(codec.replace('.','').upper(),size,lang)+' '+_name
            name = name_.replace('None','').replace("u'",'').replace("'",'')
            changeboost4 = 'http://'+self.Settings().get_setting('easy_user')+':'+self.Settings().get_setting('easy_pass')+'@'
            if self.Settings().get_setting('ssl') == 'true':
                    url = str(changeboost4).replace('http://','https://')+str(url).replace(' ','%20')
            else:
                    url = str(changeboost4)+str(url).replace(' ','%20')
