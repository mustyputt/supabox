...
class PrimeWire(MovieIndexer, MovieSource, TVShowIndexer, TVShowSource, CustomSettings):
    ...
    def __init__(self):
        ...
        xml += '<setting id="custom_url" type="labelenum" label="URL" default="http://www.primewire.ag/" values="Custom|http://www.1channel.ch/|http://www.primewire.ag/|http://www.letmewatchthis.ch/|http://1channel.occupyuk.co.uk/|http://primewireproxy.co.uk/" />\n'
    ...
    def GetContent(self, indexer, url, title, name, year, season, episode, type, list):      
        ...
        if type == 'tv_seasons':
            for item in re.finditer('<h2><a href="(.+?)">Season ([0-9]+)</a></h2>', content):
            ...
            for item in re.finditer('<div class="tv_episode_item"> <a href="(.+?)">Episode ([0-9]+)(?:.+?<span class="tv_episode_name"> - (.+?)</span>)?', content):
                item_url = item.group(1)
                if item_url[0] == '/':
                    item_url = item_url[1:]
                    item_url = custom_url + item_url
