...
### ############################################################################################################
### ############################################################################################################
SiteName='[COLOR cornflowerblue]Cartoon [COLOR orange]World[/COLOR][/COLOR]  [v0.1.3]  [Cartoons]'
...
def Browse_Hosts(url, title, img):
	url_content=nURL(url); url_content=_addon.unescape(url_content); url_content=unescape(url_content)
	if re.search(r'<div id="vid', url_content):
		for vid in re.finditer(r'<div id="vid([0-9]{1,2})"(.+?)</div>', url_content):
			vid_part=str(int(vid.group(1)) + 1); vid_lnks=vid.group(2)
