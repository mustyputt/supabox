...
def CATEGORIES():
    ...
    if addone not in show: CATEGORIES2()
        ...
        if dialog.yesno(AddonTitle,"" , "This tool is intended for Advanced Users Only!",'Brought to you by Flaggsoft','Continue','Exit'): exit
        else: CATEGORIES2()
def CATEGORIES2():
   ...
   #addDir("How-To Videos",howtourl,1,getArt('howtovideos.jpg'),defaultfanart)
    ...
    addDir('Download Supabox Media',xbmcdownload,44,getArt('downloadxbmc.jpg'),defaultfanart)
...
def ERASELOGS(url):  
    ...
    if dialog.yesno("Delete Old Crash Logs", '', "Do you want to delete them?"):
        ...
        for infile in os.walk(path, 'xbmc_crashlog*.*'):
             File=infile
             print infile
             os.remove(infile)
             dialog = xbmcgui.Dialog()
...
def CONFIGWIZARD(url):
    pluginpath = os.path.exists(xbmc.translatePath(os.path.join('special://home','addons','plugin.video.supaboxwizard')))
    if pluginpath: xbmc.executebuiltin("RunAddon(plugin.video.supaboxwizard)")
    else:
...
def XBMCDOWNLOAD():
    if local.getSetting('xbmc_path')=='':
             ...
             addDir('STABLE','http://andersonflagg.com/app/',44,getArt('stable.jpg'),defaultfanart)   
def XBMCDOWNLOAD2():
    addDir('Android','http://andersonflagg.com/app/',44,getArt('android.jpg'),defaultfanart)
def XBMCDOWNLOAD5(url):
    addDir('latest Supabox','http://andersonflagg.com/app/',41,getArt('android.jpg'),defaultfanart)
    url1=url; match=re.compile('<li><a href="(.+?)apk"> (.+?)</a>').findall(net.http_GET(url).content)
    for url,name in match:
        nono=['Name']; print name
        if name not in nono: print url1+url; addDir('%s' %(name),url1+url,42,'','')
     
def XBMCDOWNLOAD6(url):
     addDir('Android','http://andersonflagg.com/app/',47,getArt('android.jpg'),defaultfanart)

def XBMCDOWNLOAD3(url):
    print url; url1=url; match=re.compile('<li><a href="(.+?apk)"> (.+?)</a>').findall(net.http_GET(url).content)
    for url,name in match:
        nono=['Name']; print name+"downloads3"
        if name not in nono: print url1+url; addDir('%s' %(name),url1+url+'@'+url1,42,'','')
