...
def ps(x):
	return {
		...
		,'LI.movies.match.items.old': 		'class="coverImage" title="(.+?)".+?href="(.+?)".+?src="(.+?)".+?<a title=".+?\(([\d]+)\)'
		,'LI.movies.match.items': 				'class="coverImage" title="(.+?)(?: \(\d\d\d\d\))?".+?href="(.+?)".+?data-original="(.+?)".+?<a title=".+?\(([\d]+)\)'
		,'LI.movies.match.items2': 				'class="coverImage" title="(.+?)(?: \(\d\d\d\d\))?"\s+href="(.+?)">.+?src="(http://static\d*\.solarmovie\.\D+/images/movies/\d+_\d+x\d+\.jpg)".+?<a\stitle=".+?\(([\d]+)\)'
		,'b2LI.movies.match.items2': 				'class="coverImage" title="(.+?)(?: \(\d\d\d\d\))?"\s+href="(.+?)">.+?src="(http://static\d*\.solarmovie\.so/images/movies/\d+_\d+x\d+\.jpg)".+?<a\stitle=".+?\(([\d]+)\)'
		,'bLI.movies.match.items2': 				'class="coverImage" title="(.+?)"\s+href="(.+?)">.+?src="(http://static\.solarmovie\.so/images/movies/\d+_\d+x\d+\.jpg)".+?<a\stitle=".+?\(([\d]+)\)'
		,'aLI.movies.match.items': 				'class="coverImage" title="(.+?)".+?href="(.+?)".+?data-original="(.+?)".+?<a title=".+?\(([\d]+)\)'
		,'aLI.movies.match.items2': 			'class="coverImage" title="(.+?)"[\n]\s+href="(.+?)">.+?src="(http://static\.solarmovie\.so/images/movies/\d+_\d+x\d+\.jpg)".+?<a\stitle=".+?\(([\d]+)\)'
		...
		,'LI.tv.latest.split1': 					'<h2 class="clear">Most Popular New TV Shows</h2>'
		,'LI.tv.latest.split2': 					'<h3>'
		,'LI.tv.popular.all.check': 			'Popular'
		,'LI.tv.popular.all.split1': 			'<h2 class="clear">Most Popular TV Shows</h2>'
		,'LI.tv.popular.all.split2': 			'<h2>'
		,'LI.tv.popular.new.check': 			'NewPopular'
		,'LI.tv.popular.new.split1': 			'<h2 class="clear">Latest TV Shows</h2>'
		,'LI.tv.popular.new.split2': 			'<h3>'
		,'LI.tv.match.items': 						'class="coverImage" title="(.+?)(?: \(\d\d\d\d\))?".+?href="(.+?)".+?data-original="(.+?)".+?<a title=".+?\(([\d]+)\)'
		,'LI.tv.match.items.old': 				'class="coverImage" title="(.+?)(?: \(\d\d\d\d\))?".+?href="(.+?)".+?src="(.+?)".+?<a title=".+?\(([\d]+)\)'
		,'LI.nextpage.name': 							'  [COLOR goldenrod]>  [COLOR red]Next[/COLOR]...[/COLOR]'
		,'LI.nextpage.match': 						'<li class="next"><a href=.+?page=([\d]+)"'
		,'LI.nextpage.check': 						'<li class="next"><a href="http://'
		,'LI.page.param': 								'?page='
		...
		,'LLinks.compile.imdb.url_id': 		 '<a href="(http://anonym.to/.http%3A%2F%2Fwww.imdb.com%2Ftitle%2Ftt(\d+)%2F)">'
		,'LLinks.compile.imdb.url_id.old': '<strong>IMDb ID:</strong>[\n]\s+<a href="(.+?)">(\d+)</a>'
